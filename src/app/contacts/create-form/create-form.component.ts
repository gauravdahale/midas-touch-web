...
export class CreateFormComponent {
 ...
 public websiteUrl:boolean=false;

 orgform=this.fb.group({
  itemsorg:this.fb.array([])
...
webform=this.fb.group({
  webitems:this.fb.array([])

 });


get webitems(){
  return this.webform.get('webitems') as FormArray
 }
...
deletewebItems(index1:number){
  this.webitems.removeAt(index1);
